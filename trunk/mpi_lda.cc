...
namespace learning_lda {
  ...
  // TODO(yiwang): check this function is compatible with OPENMP extension.
  void AllReduceTopicDistribution(int64* buf, int count) {
    ...
    if (count > kMaxDataCount) {
      ...
      // If count is not divisible by kMaxDataCount, there are some
      // elements left to be reduced.
      ...
      MPI_Allreduce(buf, tmp_buf, count,
  ...
  int DistributelyLoadAndInitTrainingCorpus(const string& corpus_file,
                                            int num_topics,
                                            int myid,
                                            int pnum,
                                            LDACorpus* corpus,
    ...
    while (getline(fin, line)) {  // Each line is a training document.
      if (line.size() > 0 &&      // Skip empty lines.
        ...
        if (index % pnum == myid) {
          ...
          // This is a document that should be stored by other
          // processors. I just need to read the words and build the
          // word set.
          string word;
          int count;
  ...
  for (int iter = 0;
    ...
    if (flags.compute_loglikelihood_) {
      ...
      for (LDACorpus::const_iterator iter = corpus.begin();
           iter != corpus.end();
           ++iter) {
        loglikelihood_local += sampler.ComputeLogLikelihood(*iter);
      }
    ...
    sampler.DoGibbsSampling(&corpus, true, false);
  }
  ParallelLDAModel model(flags.num_topics_, word_index_map);
  model.ComputeAndAllReduce(corpus);
  if (myid == 0) {
